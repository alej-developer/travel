...
class EasyJetScraper(BaseScraper):
    """Scrapes flights from easyjet.com — European low-cost carrier."""

    domain = "easyjet.com"

    async def scrape(self, page: Page, request: ScrapeRequest) -> ScrapeResult:
        url = (
            f"https://www.easyjet.com/es/cheap-flights/{request.origin.lower()}"
            f"-{request.destination.lower()}"
            f"?date={request.departure_date}"
            f"&adults={request.adults}"
        )

        logger.info("[EasyJetScraper] GET %s", url)
        ...
        try:
            ...
            # Accept cookies
            try:
                await page.click('#ensAcceptAll, [data-testid="accept-cookies"]', timeout=5_000)
            except Exception:
                pass

            await page.wait_for_selector(
                '.flight-grid-card, [data-testid="flight-row"]', timeout=30_000
            )

            cards = await page.query_selector_all(
                '.flight-grid-card, [data-testid="flight-row"]'
            )
            for card in cards:
                try:
                    dep_el = await card.query_selector('.departure-time, [data-testid="dep-time"]')
                    arr_el = await card.query_selector('.arrival-time, [data-testid="arr-time"]')
                    price_el = await card.query_selector('.price, [data-testid="price"]')
                    duration_el = await card.query_selector('.duration, [data-testid="duration"]')
            ...
            logger.exception("[EasyJetScraper] Fatal error")
